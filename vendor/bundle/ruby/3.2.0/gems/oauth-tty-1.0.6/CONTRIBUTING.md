...
Executables shipped by oauth-tty can be used with or without generating the binstubs.
They will work when oauth-tty is installed globally (i.e., `gem install oauth-tty`) and do not require that oauth-tty be in your bundle.
...
However, the rake tasks provided by oauth-tty do require oauth-tty to be added as a development dependency and loaded in your Rakefile.
See the full list of rake tasks in head of Rakefile

**Gemfile**
```ruby
group :development do
  gem "oauth-tty", require: false
end
```
...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/oauth/tty/release_cli.rb` -> `spec/oauth/tty/release_cli_spec.rb`.
...
[🤝conduct]: https://gitlab.com/ruby-oauth/oauth-tty/-/blob/main/CODE_OF_CONDUCT.md
